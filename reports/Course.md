...
На рис. \ref{fig:condense} представлен конденсированный граф переходов, представляющий различные пути прохождения квеста.
...
Для краткости часть текста опущена многоточием. Полную таблицу фактов см. в приложении.

(Также, дерево содержит три конечных узла. См. [Предметная область].)

На рисунке \ref{} представлен полный орграф решений, текст в узлах заменен идентификаторами.
...
Реализация данной экспертной системы будет активно использовать
...
вопроса (\"Предпочитаете помпезность и роскошь?\"), пользователь
переходит от вопроса к вопросу, с помощью элементарных ответов «Да» или
«Нет», пока не дойдет до конечного факта.
...
бинарного дерева. Пользователь с помощью ответов yes/no отвечает на все
последующие вопросы и получает рекомендацию о подходящем архитектурном
стиле для его загородного дома. В случае повторного использования
