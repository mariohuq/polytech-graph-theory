...
переходит от блока к блоку с помощью выбора варианта вводом одного символа из перечня `abcd`,
пока не дойдет до конечного блока.

Начальный узел определяется следующими фактами:
...
```{.clp include=../Course/hobbit.clp snippet=first-fact}
```

# Результаты работы программы

На рис. \ref{bad 1}-\ref{} представлены результаты работы программы.

После запуска программы выводится первый блок (начальная вершина орграфа).
Пользователь с помощью ответов `a/b/c/d` производит выбор и приходит к одной из концовок.
